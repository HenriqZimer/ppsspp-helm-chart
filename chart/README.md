...
[![Version: 1.0.2](https://img.shields.io/badge/Version-1.0.2-informational?style=flat-square)](https://github.com/HenriqZimer/ppsspp-helm-chart)
...
PlayStation Portable emulator, served as a full desktop over the browser via KasmVNC.
...
```bash
git clone https://github.com/HenriqZimer/ppsspp-helm-chart.git
cd ppsspp-helm-chart

helm package chart/
helm install ppsspp ./ppsspp-1.0.2.tgz
```
...
### PPSSPP crashes when launching a game (SIGBUS)

If the dashboard/UI works but a game crashes on boot (check the container logs for
`Unhandled SIGBUS`), PPSSPP's Allegrex JIT recompiler may need more `/dev/shm` than the small
...
starts when `/dev/shm` is too small); not independently confirmed on PPSSPP, but worth trying
...
[linuxserver.io GPU Configuration docs](https://docs.linuxserver.io/images/docker-ppsspp/#gpu-configuration)):
...
If PPSSPP itself crashes with `Unhandled SIGBUS` in the container logs on boot (i.e. the dashboard works
but launching a game doesn't), see `shmSize` below and `seccompUnconfined` above — try those first, it's
usually not a GPU problem.

### RomM emulator streaming — not available for PPSSPP

The sibling PCSX2/Dolphin/xemu/Eden charts (and, experimentally, RPCS3) can run as a target for
RomM's [Emulator Streaming](https://docs.romm.app/latest/using/emulator-streaming/) feature via a
third-party Docker Mod that adds an HTTP broker RomM talks to. **No such mod exists for PPSSPP**:
[github.com/LoneAngelFayt/ppsspp-romm-integration](https://github.com/LoneAngelFayt/ppsspp-romm-integration)
is an empty placeholder repository — a `LICENSE` file, no code, no CI runs, not even an unreleased
`dev` tag to point `DOCKER_MODS` at.

The chart still carries `streaming.enabled`/`streaming.brokerPort` in `values.yaml` so its shape
matches its siblings and it's ready to use if a mod ever ships, but there is nothing to enable
today — leave it `false` and use PPSSPP directly through its own desktop (`service.httpsPort`),
opening ROMs from wherever you mounted your library (see `extraVolumes` above).
...
| `image.repository` | `lscr.io/linuxserver/ppsspp` | Container image. |
