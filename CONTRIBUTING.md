# Contributing to PPSSPP Helm Chart
...
helm template ppsspp chart/

# Install in a test cluster
helm install ppsspp-test chart/ --dry-run --debug
```
...
- Update `appVersion` in `Chart.yaml` in your PR when pinning the chart to a specific PPSSPP image
